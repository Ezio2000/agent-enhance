...
const ADOPTED = ["NODE_REPL_NODE_PATH", "CUA_REPL_NODE_REPL_PATH", "NODE_REPL_NODE_MODULE_DIRS", "CODEX_CLI_PATH", "SKY_CUA_SERVICE_PATH"] as const;
type Adopted = typeof ADOPTED[number];

const bundledProbe: LayoutProbe = {
  realpath: path => realpath(path),
  exists: async path => { try { await access(path, constants.F_OK); return true; } catch { return false; } },
  executable: async path => { try { await access(path, constants.X_OK); return true; } catch { return false; } },
  readText: path => readFile(path, "utf8"),
  entries: async path => { try { return await readdir(path); } catch { return []; } },
};
...
interface Materialized { file: string; command: string; args: string[]; env: Record<string, string> }
interface Materialization { launch?: Materialized; problems: string[] }
 ...
 */
async function materialized(pluginDir: string, probe: LayoutProbe, roots: readonly string[]): Promise<Materialization> {
  ...
  for (const name of [...pinned, ...versions]) {
    ...
    try { manifest = JSON.parse(await probe.readText(file)); }
    catch (error) {
      problems.push(`${file}: ${error instanceof SyntaxError ? "invalid JSON" : "no materialized plugin manifest"}`);
      continue;
    }
    const servers = manifest !== null && typeof manifest === "object" ? (manifest as { mcpServers?: unknown }).mcpServers : undefined;
    const raw = servers !== null && typeof servers === "object" ? (servers as Record<string, unknown>).cua_repl : undefined;
    const server = raw !== null && typeof raw === "object" ? raw as { command?: unknown; args?: unknown; env?: unknown } : undefined;
    const command = typeof server?.command === "string" ? server.command : "";
    const declared = Array.isArray(server?.args) ? server.args.filter((value): value is string => typeof value === "string") : [];
    ...
    if (!command || !launcher || !isAbsolute(command)) {
      problems.push(`${file}: no materialized cua_repl launch command (enable Computer Use in the desktop app, then restart it)`);
      continue;
    }
    ...
    if (!await probe.executable(command)) { problems.push(`${file}: ${command} is not executable`); continue; }
    if (!await probe.exists(launcher)) { problems.push(`${file}: ${launcher} is missing`); continue; }
 ...
 */
export async function resolveLayout(options: { app: string; codexHome: string; probe?: LayoutProbe }): Promise<LayoutPaths> {
  const probe = options.probe ?? bundledProbe;
  const codexHome = options.codexHome;
  let app: string;
  try { app = await probe.realpath(options.app); }
  catch { throw new Error(`Official Computer Use runtime not found: ${options.app} does not exist. Point OPENAI_CODEX_COMPUTER_APP at the installed desktop application.`); }
  ...
  if (!found.launch) {
    ...
    throw new Error(`Official Computer Use runtime not found in ${app}. ${detail}. Inspected ${pluginDir}; start the desktop app once with Computer Use enabled so it materializes the plugin. Nothing is downloaded automatically.`);
  }
  ...
  const adopt = async (key: Adopted, fallback: string, mode: "exists" | "executable"): Promise<string> => {
    ...
    if (!usable) { notes.push(`materialized ${key}=${candidate} is unusable; using ${fallback}`); return fallback; }
    return candidate;
  };
  ...
  if (!configuredService && materializedService) {
    if (isAbsolute(materializedService) && inside(materializedService, roots) && await probe.exists(materializedService)) configuredService = materializedService;
    else notes.push(`materialized SKY_CUA_SERVICE_PATH=${materializedService} is unusable; using the per-user default`);
  }
  ...
  for (const item of required) {
    if (!item.path || !await (item.executable ? probe.executable(item.path) : probe.exists(item.path))) missing.push(`${item.label} (${item.path || "unresolved"})`);
  }
  if (missing.length) {
    const detail = notes.length ? ` ${notes.join("; ")}.` : "";
    throw new Error(`Official Computer Use runtime is incomplete in ${app}: missing or not executable: ${missing.join(", ")}.${detail} Verified manifest: ${launch.file}.`);
  }
  return { app, codexHome, resources, root, command: launch.command, args: launch.args, node, repl, codex, modules, service, manifest: launch.file };
}
