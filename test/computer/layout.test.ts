...
import { resolveLayout, serviceExecutable, type LayoutPaths, type LayoutProbe } from "../../src/capabilities/computer/layout.ts";
import { runtimeEnvironment } from "../../src/capabilities/computer/runtime.ts";
...
const manifest = (command: string, args: string[], env: Record<string, string> = {}) => JSON.stringify({ mcpServers: { cua_repl: { command, args, env } } });
...
function probe(files: Record<string, string>, dirs: string[] = [], executables: string[] = []): LayoutProbe {
  ...
  return {
    realpath: async path => { if (!present.has(path)) throw new Error(`ENOENT: ${path}`); return path; },
    exists: async path => present.has(path),
    executable: async path => runnable.has(path),
    readText: async path => { const value = files[path]; if (value === undefined) throw new Error(`ENOENT: ${path}`); return value; },
    entries: async path => { const prefix = `${path}/`; return [...present].filter(entry => entry.startsWith(prefix)).map(entry => entry.slice(prefix.length)).filter(name => !name.includes("/")); },
  };
}

/** A shipped bundle plus optional materialized plugin directories. */
function environment(options: { manifests?: Array<{ version: string; body: string }>; files?: Record<string, string>; dirs?: string[]; executables?: string[] } = {}): LayoutProbe {
  const files: Record<string, string> = { [NODE]: "", [REPL]: "", [CODEX]: "", [SERVICE]: "", ...options.files };
...
test("the materialized manifest supplies the launch command, so a renamed launcher needs no extra code", async () => {
  ...
  const layout = await resolveLayout({ app: APP, codexHome: CODEX_HOME, probe: environment({
...
test("the latest pointer outranks a numerically newer version directory", async () => {
  const pinned = join(MODULES, "pinned.mjs"), newer = join(MODULES, "newer.mjs");
  const layout = await resolveLayout({ app: APP, codexHome: CODEX_HOME, probe: environment({
    manifests: [
...
test("a materialized but unlaunchable cua_repl reports that Computer Use is not enabled", async () => {
  await assert.rejects(
    resolveLayout({ app: APP, codexHome: CODEX_HOME, probe: environment({ manifests: [{ version: "26.908.40834", body: manifest("node", []) }] }) }),
    (error: Error) => { assert.match(error.message, /enable Computer Use in the desktop app/); return true; },
  );
});
...
test("a missing plugin cache reports the inspected location instead of guessing a launcher path", async () => {
  await assert.rejects(
    ...
    (error: Error) => { assert.ok(error.message.includes(PLUGIN)); assert.match(error.message, /materializes the plugin/); return true; },
  );
});

test("valid materialized env overrides are adopted and unusable ones fall back", async () => {
  const alternate = join(ROOT, "bin", "node_repl_alt"), launcher = join(MODULES, "launcher.mjs");
  const layout = await resolveLayout({ app: APP, codexHome: CODEX_HOME, probe: environment({
    manifests: [{ version: "26.908.40834", body: manifest(NODE, [launcher], { CUA_REPL_NODE_REPL_PATH: alternate, CODEX_CLI_PATH: "/nonexistent/codex" }) }],
...
test("an incomplete bundle names every path that failed verification", async () => {
  ...
  const files = { [NODE]: "", [launcher]: "", [join(PLUGIN, "26.908.40834", ".mcp.json")]: manifest(NODE, [launcher]) };
  await assert.rejects(
    resolveLayout({ app: APP, codexHome: CODEX_HOME, probe: probe(files, [APP, RESOURCES, ROOT, MODULES, join(PLUGIN, "26.908.40834")], [NODE]) }),
    (error: Error) => { assert.match(error.message, /node_repl/); assert.match(error.message, /codex/); assert.match(error.message, /Sky service/); return true; },
  );
});

test("the runtime environment stays computer-only, sanitized, and pinned to the private Sky pipe", () => {
  const layout: LayoutPaths = { app: APP, codexHome: CODEX_HOME, resources: RESOURCES, root: ROOT, command: NODE, args: [join(MODULES, "launcher.mjs")], node: NODE, repl: REPL, codex: CODEX, modules: MODULES, service: SERVICE, manifest: join(PLUGIN, "26.908.40834", ".mcp.json") };
  ...
  assert.equal(Object.keys(env).some(key => key.startsWith("BROWSER_")), false);
});
