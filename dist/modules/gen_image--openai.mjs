...
// packages/capabilities/gen_image/openai/src/manifest.ts
var manifest = {
  apiVersion: 1,
  id: "gen_image/openai",
...
// packages/capabilities/gen_image/openai/src/types.ts
var IMAGE_MODELS = ["gpt-image-2", "gpt-image-2.5-sunburst", "gpt-image-2.5-flare"];
var IMAGE_FIXED = Object.freeze({
  n: 1,
  output_format: "png",
  stream: true,
  partial_images: 0
...
var IMAGE_TIMEOUT = Object.freeze({ minSeconds: 10, defaultSeconds: 300, maxSeconds: 600 });
var IMAGE_MAX_REFERENCES = 16;
var IMAGE_SIZE_RULES = Object.freeze({
  multiple: 16,
  maxEdge: 3840,
  maxAspect: 3,
  minPixels: 655360,
...
// packages/capabilities/gen_image/openai/src/validation.ts
function validateReference(ref) {
  if (Object.keys(ref).some((key) => key !== "image_url"))
    throw new Error("Image references only support image_url.");
  requireText(ref.image_url, "image_url", 7e7);
  if (!/^data:image\/(png|jpeg|webp);base64,/i.test(ref.image_url)) {
    const url = new URL(ref.image_url);
    if (!["https:", "http:"].includes(url.protocol) || url.username || url.password)
      throw new Error(
        "Image URL must be HTTP(S), without embedded credentials, or a PNG/JPEG/WebP data URL."
      );
  }
}
function validateImageRequest(request) {
  requireText(request.prompt, "prompt");
  const allowed = /* @__PURE__ */ new Set([...Object.keys(IMAGE_DEFAULTS), "prompt", "images", "size", "background"]);
  for (const key of Object.keys(request))
    if (!allowed.has(key)) throw new Error(`Unsupported image parameter: ${key}.`);
  for (const key of Object.keys(IMAGE_FIXED)) {
    if (request[key] !== void 0 && request[key] !== IMAGE_FIXED[key])
      throw new Error(`${key} is fixed to ${IMAGE_FIXED[key]}.`);
  }
  if (request.model !== void 0 && !IMAGE_MODELS.includes(request.model))
    throw new Error("Invalid image model.");
  if (request.quality !== void 0 && !["auto", "low", "medium", "high"].includes(request.quality))
    throw new Error("Invalid image quality.");
  if (request.moderation !== void 0 && !["auto", "low"].includes(request.moderation))
    throw new Error("Invalid image moderation.");
  if (request.background !== void 0 && !["auto", "opaque", "transparent"].includes(request.background))
    throw new Error("Invalid image background.");
  if (request.images !== void 0) {
  ...
  if (request.size && request.size !== "auto") {
    ...
    if (w <= 0 || h <= 0 || w % rules.multiple || h % rules.multiple || Math.max(w, h) > rules.maxEdge || Math.max(w, h) / Math.min(w, h) > rules.maxAspect || pixels < rules.minPixels || pixels > rules.maxPixels) {
      throw new Error(
        `GPT Image size: edges must be multiples of ${rules.multiple} and <=${rules.maxEdge}, aspect ratio <=${rules.maxAspect}:1, pixels ${rules.minPixels}\u2013${rules.maxPixels}.`
      );
    }
  }
}

// packages/capabilities/gen_image/openai/src/client.ts
function parseQuotaHeaders(headers) {
  if (headers.get("x-codex-primary-used-percent") === null) return void 0;
  ...
  return {
    ...
    ...Number.isFinite(resetAfter) && resetAfter > 0 ? { resetAt: Date.now() + resetAfter * 1e3 } : {}
  };
}
var ImageClient = class {
  ...
  async images(request, options = {}) {
    ...
    request = {
      ...
      quality: request.quality ?? IMAGE_DEFAULTS.quality,
      moderation: request.moderation ?? IMAGE_DEFAULTS.moderation
    };
    let quota;
    const result = await this.http.post(
      request.images ? "images/edits" : "images/generations",
      request,
      {
        signal: options.signal,
        timeoutMs: options.timeoutMs ?? IMAGE_TIMEOUT.defaultSeconds * 1e3,
        headers: options.turnId ? { "x-codex-image-turn-id": options.turnId } : void 0,
        consume: async (response, signal, id, secrets) => {
          ...
          if (!(response.headers.get("content-type") ?? "").includes("text/event-stream")) {
            ...
            const result2 = validateImageResponse(data);
            if (request.stream)
              result2.warnings = [
                "Requested SSE streaming, but the backend returned a non-streaming JSON response."
              ];
            return result2;
          }
          let completed;
          for await (const item of readSSE(response, 192 * 1024 * 1024, signal)) {
            ...
            if (type === "error" || type.endsWith(".failed") || event.error)
              throw responseError(event, 200, id, secrets);
            if (type === "image_generation.partial_image" || type === "image_edit.partial_image") {
              options.onProgress?.({
                index: typeof event.partial_image_index === "number" ? event.partial_image_index : void 0,
                b64_json: typeof event.b64_json === "string" ? event.b64_json : void 0,
                output_format: typeof event.output_format === "string" ? event.output_format : void 0
              });
          ...
          if (!completed)
            throw new ProtocolError(
              "Image stream ended without a completed image; no partial output was treated as success."
            );
          return completed;
        }
      }
...
function validateImageResponse(data) {
  if (!isRecord(data) || !Array.isArray(data.data) || data.data.length === 0 || data.data.length > 10)
    throw new ProtocolError("Image response must contain 1\u201310 images.");
  for (const item of data.data) {
    if (!isRecord(item) || typeof item.b64_json !== "string" || !item.b64_json)
      throw new ProtocolError(
        "Image response is missing b64_json; remote output URL fetching is deliberately disabled."
      );
  }
  return data;
}

// packages/capabilities/gen_image/openai/src/artifacts.ts
import { open, realpath as realpath2, lstat as lstat2 } from "node:fs/promises";
...
// packages/capabilities/gen_image/openai/src/artifacts.ts
var MAX_IMAGE_BYTES = 50 * 1e3 * 1e3 - 1;
var MAX_TOTAL_INPUT_BYTES = 100 * 1024 * 1024;
...
function decodeImage(base64, maxBytes = MAX_IMAGE_BYTES) {
  if (base64.length > Math.ceil(maxBytes / 3) * 4 || !/^[A-Za-z0-9+/]+={0,2}$/.test(base64) || base64.length % 4 !== 0)
    throw new Error("Invalid or oversized base64 image.");
...
async function resolveImage(source, cwd, signal) {
  signal?.throwIfAborted();
  if (Object.keys(source).some((key) => !["path", "image_url"].includes(key)))
    throw new Error("Image sources only support path or image_url.");
  if ([source.path, source.image_url].filter((x) => x !== void 0).length !== 1)
    throw new Error("Each image requires exactly one of path or image_url.");
  if (source.path !== void 0) {
    let name = source.path.replace(/^@/, "");
    if (name.startsWith("~/")) name = join2(homedir(), name.slice(2));
    const path = await realpath2(resolve(cwd, name));
    if (!(await lstat2(path)).isFile()) throw new Error("Input must be a regular image file.");
    const file = await open(path, "r");
    try {
      const stat = await file.stat();
      if (!stat.isFile() || stat.size > MAX_IMAGE_BYTES)
        throw new Error("Input must be a regular PNG/JPEG/WebP file smaller than 50 MB.");
      ...
      return {
        reference: { image_url: `data:${info.mimeType};base64,${bytes.toString("base64")}` },
        bytes,
        info
      };
    } finally {
  ...
  const reference = { image_url: source.image_url };
...
var ImageArtifactStore = class extends ArtifactDirectories {
  async saveImages(sessionId, images, signal) {
    ...
    for (const [i, image] of decoded.entries()) {
      signal?.throwIfAborted();
      const path = join2(directory, `image-${i + 1}.${image.info.extension}`);
      const file = await open(path, "wx", 384);
      try {
        await file.writeFile(image.bytes, { signal });
      } finally {
        await file.close();
      }
      saved.push({
...
// packages/capabilities/gen_image/openai/src/tool.ts
...
function imageWarnings(request, response, actual = []) {
  ...
  if (response.data.length !== (request.n ?? 1))
    warnings.push(
      `Requested n=${request.n ?? 1}, but the backend returned ${response.data.length} image(s).`
    );
  const expectedFormat = request.output_format ?? "png";
  const formats2 = new Set(
    actual.length ? actual.map((image) => image.mimeType.replace("image/", "")) : response.output_format ? [response.output_format] : []
  );
  for (const format of formats2)
    if (format !== expectedFormat)
      warnings.push(
        `Requested output_format=${expectedFormat}, but the actual output is ${format}. The backend did not honor the format; originals were saved without conversion.`
      );
  if (request.size && request.size !== "auto") {
    const sizes = new Set(
      actual.filter((image) => image.width && image.height).map((image) => `${image.width}x${image.height}`)
    );
    if (!sizes.size && response.size) sizes.add(response.size);
    for (const size of sizes)
      if (size !== request.size)
        warnings.push(
          `Requested size=${request.size}, but the actual output is ${size}. The backend did not honor the dimensions.`
        );
  }
  for (const name of ["quality", "background"]) {
    if (request[name] && request[name] !== "auto" && response[name] && response[name] !== request[name])
      warnings.push(`Requested ${name}=${request[name]}, but the backend reports ${response[name]}.`);
  }
  return warnings;
}
...
// packages/capabilities/gen_image/openai/src/schema.ts
var imageSource = object({
  path: typebox_exports.Optional(
    text("Local PNG/JPEG/WebP file to upload; relative to current working directory", 4096)
  ),
  image_url: typebox_exports.Optional(
    text(
      "HTTP(S) image URL, or data:image/png|jpeg|webp;base64,...; fetched by OpenAI, not this client",
      7e7
    )
  )
});
var ImageSchema = object({
  model: typebox_exports.Optional(
    choices(
      [...IMAGE_MODELS],
      "Default gpt-image-2.5-flare. Sunburst prioritizes editing precision; Flare prioritizes fast everyday generation. Backend model routing is not independently verifiable."
    )
  ),
  prompt: text(
    "Detailed generation/edit instructions. For edits, state what to preserve and what to change.",
    32e3
  ),
  images: typebox_exports.Optional(
    typebox_exports.Array(imageSource, {
      minItems: 1,
      maxItems: IMAGE_MAX_REFERENCES,
      description: "Edit/reference inputs. Exactly one of path or image_url per item. Omit to generate a new image."
    })
  ),
  size: typebox_exports.Optional(
    text(
      `auto or WIDTHxHEIGHT. All supported models: multiples of ${IMAGE_SIZE_RULES.multiple}, edges <=${IMAGE_SIZE_RULES.maxEdge}, ratio <=${IMAGE_SIZE_RULES.maxAspect}:1, ${IMAGE_SIZE_RULES.minPixels}\u2013${IMAGE_SIZE_RULES.maxPixels} pixels.`,
      30
    )
  ),
  background: typebox_exports.Optional(
    choices(["auto", "opaque", "transparent"], "Default auto; transparent PNG output is supported")
  ),
  quality: typebox_exports.Optional(
    choices(
      ["auto", "low", "medium", "high"],
      "Default auto. Backend may adjust quality; observable mismatches are reported."
    )
  ),
  timeout_seconds: typebox_exports.Optional(
    typebox_exports.Integer({
      ...
      description: `Default ${IMAGE_TIMEOUT.defaultSeconds}. Image POSTs are never automatically retried.`
    })
  )
});
...
function formatResetIn(resetAt, now = Date.now()) {
  const total = Math.max(0, Math.round((resetAt - now) / 1e3));
  const days = Math.floor(total / 86400), hours = Math.floor(total % 86400 / 3600), minutes = Math.floor(total % 3600 / 60);
  if (days > 0) return `${days}d ${hours}h`;
  if (hours > 0) return `${hours}h ${minutes}m`;
  return `${minutes}m`;
}
function quotaLine(quota, now) {
  if (!quota || quota.usedPercent === void 0) return "";
  const parts = [`quota ${quota.usedPercent}% used`];
  if (quota.resetAt !== void 0) parts.push(`resets in ${formatResetIn(quota.resetAt, now)}`);
...
function imageWireOptions(args) {
  return {
    ...
    quality: args.quality ?? IMAGE_DEFAULTS.quality,
    moderation: IMAGE_DEFAULTS.moderation
  };
}
  ...
  return {
    ...
    async execute(callId, args, signal, onUpdate, ctx) {
      ...
      if (!value_exports.Check(ImageSchema, args))
        throw new Error(
          "Invalid gen_image arguments; unsupported or fixed parameters are rejected and this build never reads conversation context images. Use the current tool schema."
        );
      ...
      for (const source of sources) {
        ...
        if (inputBytes > MAX_TOTAL_INPUT_BYTES)
          throw new Error("Combined image inputs exceed the 100 MiB safety limit.");
        resolved.push(image);
      }
      const request = {
        ...imageWireOptions(args),
        images: resolved.length ? resolved.map((image) => image.reference) : void 0
      };
      validateImageRequest(request);
      ...
      const statusText = () => [
        ...
        [`\u23F1 ${formatElapsed(elapsedSeconds())}`, quotaLine(quota)].filter(Boolean).join(" \xB7 ")
      ].join("\n");
          ...
          onProgress: (progress) => {
            ...
            onUpdate?.({
              content: [
                {
                  type: "text",
                  text: `Received partial image${progress.index !== void 0 ? ` ${progress.index + 1}` : ""}; waiting for the final image\u2026`
                }
              ],
      ...
      const content = [
        {
          type: "text",
          text: images.map(
            (image, i) => `Image ${i + 1}: ${image.path}${image.width ? ` (${image.width}x${image.height})` : ""}`
          ).join("\n") + `
${summary}
...
${warnings.map((warning) => `- ${warning}`).join("\n")}` : "")
      ...
      return {
        ...
        details: {
          ...
          status: "completed",
          model: request.model,
          operation: request.images ? "edit" : "generate",
          requestId: result.requestId,
          images,
          warnings,
          elapsedSeconds: elapsedSeconds(),
          ...quota ? { quota } : {},
          outputFormat: result.data.output_format,
...
// packages/capabilities/gen_image/openai/src/index.ts
var index_default = {
  manifest,
  create: (services) => ({
    tool: imageTool({
