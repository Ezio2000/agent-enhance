...
// packages/capabilities/view_video/opencode/src/manifest.ts
var manifest = {
  apiVersion: 1,
...
// packages/capabilities/view_video/opencode/src/tool.ts
...
// packages/transports/opencode/src/auth.ts
async function resolveOpencodeGoAuth(ctx) {
  ...
  if (url.origin !== "https://opencode.ai" || !url.pathname.startsWith("/zen/go/") || url.username || url.password || url.search || url.hash)
...
// packages/transports/opencode/src/responses.ts
import { randomUUID } from "node:crypto";

// packages/transports/opencode/src/http.ts
...
function redact(text2, secrets = []) {
  ...
  return out.replace(/Bearer\s+[^\s"\\]+/gi, "Bearer [REDACTED]").replace(/eyJ[\w-]+\.[\w-]+\.[\w-]+/g, "[REDACTED JWT]");
}
...
var ResponsesClient = class {
  ...
  async askWithFile(args, options = {}) {
    ...
    const result = await this.http.post("responses", body, {
...
function extractText(data) {
  ...
  if (!text2) throw new Error("Backend returned an empty answer; try a larger max_output_tokens.");
  return text2;
}
...
// packages/transports/opencode/src/schema.ts
...
// packages/capabilities/view_video/opencode/src/schema.ts
...
var VideoSchema = object({
  ...
  prompt: text("What to describe or answer from the video", 4e3),
  effort: typebox_exports.Optional(choices([...EFFORTS], "Reasoning effort; default minimal")),
  max_output_tokens: typebox_exports.Optional(typebox_exports.Integer({ minimum: 64, maximum: 8192, description: "Default 1024" })),
  timeout_seconds: typebox_exports.Optional(
    typebox_exports.Integer({ minimum: 10, maximum: 300, description: "Default 120. Never automatically retried." })
  )
});
...
function videoTool(deps = {
  ...
  return {
    name: "view_video",
    label: "Muse Video",
    description: "Parse a local video file with Muse Spark via opencode-go responses protocol. Reads the file, sends it as input_file, returns the model's answer as text. No artifacts are saved. Audio-only input is not understood by the backend; this tool is video only.",
    ...
    async execute(callId, args, signal, onUpdate, ctx) {
      ...
      const result = await deps.client(ctx).askWithFile(
        ...
        { signal, timeoutMs: (args.timeout_seconds ?? 120) * 1e3 }
      );
      signal?.throwIfAborted();
      return {
...
// packages/capabilities/view_video/opencode/src/index.ts
var index_default = { manifest, create: () => ({ tool: videoTool() }) };
export {
