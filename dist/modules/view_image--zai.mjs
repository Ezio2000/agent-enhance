...
function responseError(value, status, requestId, secrets = []) {
  ...
  const detail = typeof error.message === "string" ? error.message : typeof error.msg === "string" ? error.msg : typeof root.error === "string" ? root.error : "Request rejected by the backend";
  const hint = status === 401 || code === "1001" || code === "401" ? " Reauthenticate the GLM Coding Plan in the current host." : code === "1113" ? " No subscription quota on this endpoint; confirm the key belongs to a GLM Coding Plan." : code === "1302" || code === "1303" ? " Risk-control response; stop calling and review the subscription usage rules. No retry was made." : status === 429 ? " Rate limited; no automatic retry was made." : code === "1214" ? " Check parameters." : code === "1701" ? " Search concurrency limit; retry fewer parallel queries." : code === "1703" ? " The search returned no usable results; try a different query." : "";
...
// packages/capabilities/view_image/zai/src/client.ts
var VisionClient = class {
  ...
  async analyze(request, options = {}) {
    const { data, requestId } = await this.http.post("chat/completions", request, {
...
// packages/capabilities/view_image/zai/src/tool.ts
...
var AssertError = class extends Error {
  constructor(source2, value, errors) {
    super(source2);
    Object.defineProperty(this, "cause", {
      value: { source: source2, errors, value },
      writable: false,
      configurable: false,
      enumerable: false
    });
...
// packages/capabilities/view_image/zai/src/prompts.ts
var UI_TO_ARTIFACT_PROMPTS = {
...
var IMAGE_ANALYSIS_PROMPT = `You are an advanced AI vision assistant with comprehensive image understanding capabilities. Your strength lies in being adaptable\u2014you can analyze any visual content and provide insights tailored to what the user specifically needs, whether that's identifying objects, understanding context, extracting information, or offering detailed descriptions.
...
// packages/transports/zai/src/schema.ts
...
// packages/capabilities/view_image/zai/src/schema.ts
var TASKS = [
  "ui_to_artifact",
  "extract_text",
  "diagnose_error",
...
var source = (label) => object({
  ...
  url: typebox_exports.Optional(typebox_exports.String({ minLength: 1, description: `Public HTTP(S) URL of the ${label}` }))
});
var ViewImageSchema = object({
  task: choices([...TASKS], "Analysis task; determines the specialist system prompt"),
  prompt: text("Task instructions: what to extract, generate, compare or answer", 8e3),
  image: typebox_exports.Optional(source("image (or the video file/URL for video_analysis)")),
  image2: typebox_exports.Optional(source("second image (actual state); required with ui_diff_check")),
  output_type: typebox_exports.Optional(
    choices(["code", "prompt", "spec", "description"], "Required with ui_to_artifact")
  ),
  language: typebox_exports.Optional(text("Programming-language hint for extract_text", 60)),
  diagram_type: typebox_exports.Optional(
    text("Diagram hint for understand_diagram, e.g. architecture/flowchart/uml/er-diagram/sequence", 60)
  ),
  context: typebox_exports.Optional(text("When/how the error occurred, for diagnose_error", 500)),
  thinking: typebox_exports.Optional(
    choices(["enabled", "disabled"], "Default enabled; disable for cheap mechanical reads like plain OCR")
  ),
  max_tokens: typebox_exports.Optional(
    typebox_exports.Integer({ minimum: 64, maximum: 32768, description: "Output budget; default 4096" })
  ),
  timeout_seconds: typebox_exports.Optional(typebox_exports.Integer({ minimum: 10, maximum: 600, description: "Default 180" }))
});
...
var VISION_MODEL = "glm-5.3-flash";
function systemPrompt(task, args) {
  switch (task) {
    case "ui_to_artifact":
...
async function mediaPart(source2, kind) {
  if (source2.url && /^https?:\/\//i.test(source2.url))
    return kind === "image" ? { type: "image_url", image_url: { url: source2.url } } : { type: "video_url", video_url: { url: source2.url } };
  if (!source2.path) throw new Error(`Provide either path or url for the ${kind}.`);
  const extension = ext(source2.path);
  ...
  const bytes = await readFile(source2.path);
  const limit = kind === "image" ? MAX_IMAGE_BYTES : MAX_VIDEO_BYTES;
  if (bytes.length > limit) throw new Error(`${kind} exceeds ${limit / 1024 / 1024} MB (${source2.path}).`);
  return {
...
function viewImageTool(deps) {
  return {
    ...
    async execute(_callId, args, signal, onUpdate, ctx) {
      signal?.throwIfAborted();
      if (!value_exports.Check(ViewImageSchema, args))
      ...
      const parts = [await mediaPart(primary, wantsVideo ? "video" : "image")];
      ...
      const result = await deps.client(ctx).analyze(
        ...
        { signal, timeoutMs: (args.timeout_seconds ?? 180) * 1e3 }
      );
      signal?.throwIfAborted();
      return {
...
// packages/transports/zai/src/auth.ts
var ALLOWED_ORIGINS = ["https://api.z.ai", "https://open.bigmodel.cn"];
var DEFAULT_ORIGIN = "https://api.z.ai";
var CODING_PATH = "/api/coding/paas/v4";
function zaiCodingBase(raw) {
  const origin = (() => {
...
async function resolveZaiAuth(ctx) {
