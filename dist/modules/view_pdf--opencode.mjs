...
// packages/capabilities/view_pdf/opencode/src/manifest.ts
var manifest = {
  apiVersion: 1,
...
// packages/capabilities/view_pdf/opencode/src/tool.ts
...
// packages/capabilities/view_pdf/opencode/src/schema.ts
var EFFORTS = ["minimal", "low", "medium", "high", "xhigh"];
var PdfSchema = object({
  path: text("Local PDF file to parse; relative to current working directory", 4096),
  prompt: text("What to extract or answer from the PDF", 4e3),
...
function pdfTool(deps = {
  client: (ctx) => new ResponsesClient(() => resolveOpencodeGoAuth(ctx))
}) {
  return {
    name: "view_pdf",
    label: "Muse PDF",
    description: "Parse a local PDF with Muse Spark via opencode-go responses protocol. Reads the file, sends it as input_file, returns the model's answer as text. No artifacts are saved. Audio is not supported by the backend and is rejected; this tool is PDF only.",
    ...
    async execute(callId, args, signal, onUpdate, ctx) {
      signal?.throwIfAborted();
      if (!value_exports.Check(PdfSchema, args))
...
// packages/capabilities/view_pdf/opencode/src/index.ts
var index_default = { manifest, create: () => ({ tool: pdfTool() }) };
export {
