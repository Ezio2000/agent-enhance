...
// packages/capabilities/gen_image/xai/src/validation.ts
function validateImageArgs(args) {
  ...
  if (args.quality !== void 0 && (args.model ?? IMAGE_DEFAULTS.model) !== "grok-imagine-image-2.0") {
...
function imageWireOptions(args, references = []) {
  return {
    ...IMAGE_DEFAULTS,
    ...
    ...references.length === 1 ? { image: references[0] } : references.length > 1 ? { images: references } : {}
  };
}
...
"${promptSnippetText(request.prompt)}" \xB7 ${request.model} \xB7 elapsed ${formatElapsed(elapsed())}
Original files are saved. Previews may be resized; use original paths for subsequent edits.`
        }
      ];
      if (deps.preview) {
        try {
          const preview = await deps.preview(await readFile2(image.path), image.mimeType);
