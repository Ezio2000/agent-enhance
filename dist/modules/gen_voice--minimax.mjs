...
// packages/capabilities/gen_voice/minimax/src/manifest.ts
var manifest = {
  apiVersion: 1,
...
// packages/capabilities/gen_voice/minimax/src/types.ts
var VOICE_MODELS = ["speech-2.8-hd", "speech-2.8-turbo"];
var EMOTIONS = [
  "happy",
  "sad",
  "angry",
...
var VOICE_DEFAULTS = Object.freeze({
  model: "speech-2.8-hd",
...
var TEXT_MAX_CHARS = 1e4;
var VOICE_TIMEOUT = Object.freeze({ minSeconds: 10, defaultSeconds: 60, maxSeconds: 300 });

// packages/capabilities/gen_voice/minimax/src/validation.ts
function validateVoiceRequest(request) {
  if (typeof request.text !== "string" || !request.text.trim() || request.text.length > 1e4)
    throw new Error("text must be nonempty and at most 10000 characters.");
  const allowed = /* @__PURE__ */ new Set(["model", "text", "stream", "voice_setting", "audio_setting"]);
  for (const key of Object.keys(request))
    if (!allowed.has(key)) throw new Error(`Unsupported parameter: ${key}.`);
  if (request.model !== void 0 && !VOICE_MODELS.includes(request.model))
    throw new Error("Invalid MiniMax speech model.");
  const vs = request.voice_setting;
  const allowedVs = /* @__PURE__ */ new Set(["voice_id", "speed", "vol", "pitch", "emotion"]);
  ...
  if (vs.emotion !== void 0 && !EMOTIONS.includes(vs.emotion))
    throw new Error(`emotion must be one of: ${EMOTIONS.join(", ")}.`);
  const audio = request.audio_setting;
  if (audio.sample_rate !== 32e3 || audio.bitrate !== 128e3 || audio.format !== "mp3" || audio.channel !== 1)
    throw new Error("audio_setting is fixed to 32 kHz mono MP3 at 128 kbps.");
}

// packages/capabilities/gen_voice/minimax/src/client.ts
var numberField = (value) => typeof value === "number" && Number.isFinite(value) ? value : void 0;
var VoiceClient = class {
  ...
  async speak(request, options = {}) {
    ...
    return this.http.post("t2a_v2", request, {
      signal: options.signal,
      timeoutMs: options.timeoutMs ?? VOICE_TIMEOUT.defaultSeconds * 1e3,
      consume: async (response, signal, requestId, secrets) => {
        ...
        const extra = isRecord(root.extra_info) ? root.extra_info : void 0;
        const extraInfo = {
          ...numberField(extra?.audio_length) !== void 0 ? { audioLength: numberField(extra?.audio_length) } : {},
          ...numberField(extra?.audio_sample_rate) !== void 0 ? { audioSampleRate: numberField(extra?.audio_sample_rate) } : {},
          ...numberField(extra?.audio_size) !== void 0 ? { audioSize: numberField(extra?.audio_size) } : {},
          ...numberField(extra?.bitrate) !== void 0 ? { bitrate: numberField(extra?.bitrate) } : {},
          ...numberField(extra?.word_count) !== void 0 ? { wordCount: numberField(extra?.word_count) } : {},
          ...numberField(extra?.usage_characters) !== void 0 ? { usageCharacters: numberField(extra?.usage_characters) } : {},
          ...typeof extra?.audio_format === "string" ? { audioFormat: extra.audio_format } : {},
          ...numberField(extra?.audio_channel) !== void 0 ? { audioChannel: numberField(extra?.audio_channel) } : {}
        };
        return {
          audio,
          extraInfo,
...
// packages/capabilities/gen_voice/minimax/src/artifacts.ts
...
// packages/capabilities/gen_voice/minimax/src/artifacts.ts
var VoiceArtifactStore = class extends ArtifactDirectories {
  async saveVoice(sessionId, bytes, signal) {
    signal?.throwIfAborted();
    ...
    const path = join2(directory, "voice-1.mp3");
...
// packages/capabilities/gen_voice/minimax/src/schema.ts
var VoiceSchema = object({
  text: text(
    `Text to synthesize, at most ${TEXT_MAX_CHARS} characters. Pronunciation dictionaries and pause markers are not exposed.`,
    TEXT_MAX_CHARS
  ),
  model: typebox_exports.Optional(choices(VOICE_MODELS, "Default speech-2.8-hd (higher fidelity); turbo is faster.")),
  voice_id: typebox_exports.Optional(
  ...
  emotion: typebox_exports.Optional(choices(EMOTIONS, "Optional emotional rendering; supported by speech-2.8 models.")),
  speed: typebox_exports.Optional(typebox_exports.Number({ minimum: 0.5, maximum: 2, description: "Speech speed; default 1.0." })),
  vol: typebox_exports.Optional(typebox_exports.Number({ minimum: 0, maximum: 10, description: "Volume; default 1.0." })),
  pitch: typebox_exports.Optional(
    typebox_exports.Number({ minimum: -12, maximum: 12, description: "Pitch offset in semitones; default 0." })
  ),
...
// packages/capabilities/gen_voice/minimax/src/tool.ts
var SNIPPET_LENGTH = 60;
...
function voiceWireOptions(args) {
  return {
    ...
    voice_setting: {
      ...
      ...args.emotion === void 0 ? {} : { emotion: args.emotion }
    },
    audio_setting: {
...
function voiceTool(deps) {
  return {
    ...
    async execute(callId, args, signal, onUpdate, ctx) {
      signal?.throwIfAborted();
      if (!value_exports.Check(VoiceSchema, args))
      ...
      try {
        result = await deps.client(ctx).speak(request, {
          signal,
          timeoutMs: (args.timeout_seconds ?? VOICE_TIMEOUT.defaultSeconds) * 1e3
        });
      } catch (error) {
        throw annotateError(
          error,
          `
Text: "${snippet(request.text)}" \xB7 elapsed ${formatElapsed(elapsedSeconds())}`
        );
      }
      signal?.throwIfAborted();
...
// packages/capabilities/gen_voice/minimax/src/index.ts
var index_default = {
  manifest,
  create: (services) => ({
    tool: voiceTool({
