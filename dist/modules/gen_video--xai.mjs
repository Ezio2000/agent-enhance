...
// packages/capabilities/gen_video/xai/src/types.ts
var VIDEO_MODEL = "grok-imagine-video-1.5";
var VIDEO_ASPECT_RATIOS = ["1:1", "16:9", "9:16", "4:3", "3:4", "3:2", "2:3"];
var VIDEO_RESOLUTIONS = ["480p", "720p"];
var VIDEO_DEFAULTS = { model: VIDEO_MODEL, duration: 6, resolution: "480p" };
var IMAGE_TO_VIDEO_DURATIONS = [6, 10];
var REFERENCE_DURATION = { minSeconds: 1, maxSeconds: 15 };
var VIDEO_LIMITS = { images: 14, voices: 3, keyframes: 4 };
var VIDEO_TIMEOUT = {
  minSeconds: 10,
  defaultSeconds: 300,
  maxSeconds: 600,
...
var VIDEO_POLL_INTERVAL_MS = 5e3;

// packages/capabilities/gen_video/xai/src/client.ts
...
function httpError(status, detail, kind) {
  const hint = status === 401 ? " Reauthenticate xAI in the current host to refresh credentials." : status === 429 && /0\s*\/\s*0/.test(detail) ? " This token currently has a zero model rate limit; it does not prove your SuperGrok weekly allowance is exhausted. Check subscription recognition and refresh xAI login in the current host." : "";
  return new Error(`Grok video ${kind} HTTP ${status}: ${String(detail).slice(0, 1500)}${hint}`);
}
var VideoClient = class {
  constructor(auth, fetcher = fetch, pollIntervalMs = VIDEO_POLL_INTERVAL_MS) {
    this.auth = auth;
    this.fetcher = fetcher;
    this.pollIntervalMs = pollIntervalMs;
  }
  async videos(request, options = {}) {
    const timeoutMs = options.timeoutMs ?? VIDEO_TIMEOUT.defaultSeconds * 1e3;
    const controller = new AbortController();
    const timer = setTimeout(
      () => controller.abort(
        new Error(
          `Grok video request timed out after ${timeoutMs / 1e3}s. The server may still be processing it; no automatic retry was made.`
        )
      ),
      timeoutMs
    );
  ...
  async download(url, signal) {
    const timeout = AbortSignal.timeout(VIDEO_TIMEOUT.downloadSeconds * 1e3);
...
// packages/capabilities/gen_video/xai/src/artifacts.ts
import { mkdir, mkdtemp, writeFile, rm } from "node:fs/promises";
import { join } from "node:path";
var VideoArtifactStore = class {
  constructor(root) {
    this.root = root;
  }
  async saveVideo(sessionId, bytes, signal) {
    signal?.throwIfAborted();
...
async function resolveImage(source, cwd, signal) {
  ...
  if (source.path !== void 0) {
    ...
    if (path.startsWith("~/")) path = join2(homedir(), path.slice(2));
...
var imageSource = typebox_exports.Object(
  {
    ...
    image_url: typebox_exports.Optional(
      typebox_exports.String({ minLength: 1, description: "Public HTTP(S) URL or base64 data:image/... URL." })
    )
  },
  { additionalProperties: false }
);
var ImageSchema = typebox_exports.Object(
  {
    prompt: typebox_exports.String({
      minLength: 1,
      maxLength: 32e3,
      description: "Detailed generation instructions. For edits, explicitly describe what to preserve and change."
    }),
    model: typebox_exports.Optional(choices(IMAGE_MODELS, "Default grok-imagine-image-2.0.")),
    images: typebox_exports.Optional(
      typebox_exports.Array(imageSource, {
        minItems: 1,
        maxItems: 5,
        description: "Reference images for editing. Exactly one of path or image_url per item. Omit for text-to-image."
      })
    ),
    ...
    resolution: typebox_exports.Optional(choices(["1k", "2k"], "Default 1k.")),
...
// packages/capabilities/gen_video/xai/src/schema.ts
var choices2 = (values, description) => typebox_exports.Unsafe({ type: "string", enum: [...values], description });
var VideoSchema = typebox_exports.Object(
  {
    prompt: typebox_exports.Optional(
      typebox_exports.String({
        maxLength: 32e3,
        description: "Guide the animation or describe the desired video. Optional for image-to-video; required for reference-to-video. Tag style references as <IMAGE_i> and voices as <AUDIO_0> in upload order: image, images, keyframes, last_frame."
      })
    ),
    image: typebox_exports.Optional(imageSource),
    images: typebox_exports.Optional(
      typebox_exports.Array(imageSource, {
        minItems: 1,
        maxItems: VIDEO_LIMITS.images,
        description: "Style/content references (people, objects, clothing, settings) \u2014 they appear re-rendered, not as literal frames. Up to 14. Each item is exactly one of path or image_url."
      })
    ),
    last_frame: typebox_exports.Optional(imageSource),
    keyframes: typebox_exports.Optional(
      typebox_exports.Array(
        typebox_exports.Object(
          {
            image: imageSource,
            timestamp_s: typebox_exports.Number({
              description: "Time in seconds at which the image appears, strictly inside the clip (0 < t < duration). Snapped server-side to a 1/3-second grid."
            })
          },
          { additionalProperties: false }
        ),
    ...
    voices: typebox_exports.Optional(
      typebox_exports.Array(typebox_exports.String({ minLength: 1 }), {
        minItems: 1,
        maxItems: VIDEO_LIMITS.voices,
        description: "Preset voice identifiers the subjects speak in, up to 3 (e.g. ara, eve, leo, rex). Tag them in the prompt as <AUDIO_0>."
      })
    ),
    aspect_ratio: typebox_exports.Optional(
      choices2(
    ...
    duration: typebox_exports.Optional(
      typebox_exports.Integer({
        ...
        description: "Image-to-video: 6 or 10 seconds (default 6). Reference-to-video: 1\u201315 seconds (default 6)."
      })
    ),
    resolution: typebox_exports.Optional(
      choices2(VIDEO_RESOLUTIONS, "Default 480p. Only set 720p when the user asks for higher quality.")
    ),
    timeout_seconds: typebox_exports.Optional(
      typebox_exports.Integer({
        minimum: VIDEO_TIMEOUT.minSeconds,
        maximum: VIDEO_TIMEOUT.maxSeconds,
        description: "Start+poll deadline in seconds, default 300. Download has a separate 120s budget. Requests are not automatically retried."
      })
    )
  },
  { additionalProperties: false }
...
// packages/capabilities/gen_video/xai/src/validation.ts
function videoMode(args) {
  const hasRefs = (args.images?.length ?? 0) > 0 || (args.voices?.length ?? 0) > 0 || args.last_frame !== void 0 || (args.keyframes?.length ?? 0) > 0;
  return args.image && !hasRefs ? "image_to_video" : "reference_to_video";
}
function requireOneSource(source, label) {
  if (Number(source.path !== void 0) + Number(source.image_url !== void 0) !== 1) {
    throw new Error(`${label} needs exactly one of path or image_url.`);
  }
}
function validateVideoArgs(args) {
  if (!value_exports.Check(VideoSchema, args))
    throw new Error("Invalid gen_video arguments. Use the current tool schema.");
  const sources = [
    [args.image, "image"],
    [args.last_frame, "last_frame"],
    ...(args.images ?? []).map((source, i) => [source, `images[${i}]`]),
    ...(args.keyframes ?? []).map(
      (frame, i) => [frame.image, `keyframes[${i}].image`]
    )
  ];
  for (const [source, label] of sources) if (source) requireOneSource(source, label);
  const mode = videoMode(args);
  if (mode === "image_to_video") {
    if (args.duration !== void 0 && !IMAGE_TO_VIDEO_DURATIONS.includes(args.duration)) {
  ...
  if (!args.image && !args.images?.length && !args.voices?.length && !args.last_frame && !args.keyframes?.length) {
    throw new Error(
  ...
  if (args.aspect_ratio === void 0) throw new Error("`aspect_ratio` is required for reference-to-video.");
...
// packages/capabilities/gen_video/xai/src/tool.ts
async function resolveUrl(source, cwd, signal) {
  return (await resolveImage(source, cwd, signal)).url;
}
function videoWireOptions(args, resolved) {
  const mode = videoMode(args);
  const image = resolved.image ? { url: resolved.image } : void 0;
  return {
    model: VIDEO_MODEL,
    ...
    ...resolved.referenceImages.length ? { reference_images: resolved.referenceImages.map((url) => ({ url })) } : {},
    ...resolved.voices.length ? { reference_audios: resolved.voices.map((voice_id) => ({ voice_id })) } : {},
...
function videoTool(deps) {
  return {
    name: "gen_video",
    label: "Grok Video",
    description: "Generate one mp4 with Grok Imagine Video using the configured xAI credentials, independently of the main model provider. Two modes share this tool. Image-to-video: pass `image` (local PNG/JPEG/WebP path or HTTP(S)/data URL) and optionally `prompt`; duration is 6 or 10 seconds (default 6); aspect ratio follows the source and is omitted. Reference-to-video: required `prompt` plus at least one of `image` (literal first frame), `images` (up to 14 style/content refs), `voices` (up to 3), `last_frame`, or `keyframes` (up to 4 interior pins); required `aspect_ratio`; duration 1\u201315 seconds (default 6). Resolution 480p (default) or 720p. Model grok-imagine-video-1.5 is fixed. Inspect references before use. Saves the original mp4 locally. Each call returns one video, can take several minutes, and consumes account quota. Requests are not automatically retried.",
    ...
    async execute(_callId, args, signal, onUpdate, ctx) {
      signal?.throwIfAborted();
      const mode = validateVideoArgs(args);
      const image = args.image ? await resolveUrl(args.image, ctx.cwd, signal) : void 0;
      const referenceImages = [];
      for (const source of args.images ?? []) referenceImages.push(await resolveUrl(source, ctx.cwd, signal));
      const lastFrame = args.last_frame ? await resolveUrl(args.last_frame, ctx.cwd, signal) : void 0;
      const keyframes = [];
      for (const frame of args.keyframes ?? [])
        keyframes.push({
...
"${promptSnippetText(request.prompt || "(no prompt)")}" \xB7 ${request.model} \xB7 ${request.resolution} \xB7 elapsed ${formatElapsed(elapsed())}
...
// packages/transports/xai/src/auth.ts
async function resolveGrokAuth(ctx) {
