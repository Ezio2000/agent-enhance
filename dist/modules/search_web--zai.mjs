...
// packages/capabilities/search_web/zai/src/manifest.ts
var manifest = {
  apiVersion: 1,
  id: "search_web/zai",
...
// packages/capabilities/search_web/zai/src/refs.ts
var RefStore = class {
...
// packages/capabilities/search_web/zai/src/schema.ts
...
var ZaiWebSchema = object({
  ...
  open: typebox_exports.Optional(
    typebox_exports.Array(
      object({
        ...
        lineno: typebox_exports.Optional(
          typebox_exports.Integer({ minimum: 0, description: "Accepted for symmetry; ignored by zai" })
        )
      }),
      { minItems: 1, maxItems: 10 }
    )
  ),
  search_engine: typebox_exports.Optional(
    choices(
  ...
  no_gfm: typebox_exports.Optional(typebox_exports.Boolean({ description: "Disable GitHub Flavored Markdown; default false" })),
  keep_img_data_url: typebox_exports.Optional(
    typebox_exports.Boolean({ description: "Keep inline image data URLs; default false" })
  ),
  with_images_summary: typebox_exports.Optional(
    typebox_exports.Boolean({ description: "Append an image summary for opened pages; default false" })
  ),
  with_links_summary: typebox_exports.Optional(
    typebox_exports.Boolean({ description: "Append a link summary for opened pages; default false" })
  ),
  reader_timeout: typebox_exports.Optional(
    typebox_exports.Integer({ minimum: 1, maximum: 120, description: "Per-page fetch timeout in seconds; default 20" })
  ),
  timeout_seconds: typebox_exports.Optional(
    typebox_exports.Integer({ minimum: 5, maximum: 300, description: "Whole-call budget; default 90" })
  )
});

// packages/capabilities/search_web/zai/src/types.ts
...
// packages/capabilities/search_web/zai/src/tool.ts
import { open } from "node:fs/promises";
import { join as join2 } from "node:path";
var ZaiOutputStore = class extends ArtifactDirectories {
...
function formatResults(refs, results) {
...
function zaiWebTool(deps) {
  ...
  return {
    ...
    async execute(_callId, args, signal, onUpdate, ctx) {
      signal?.throwIfAborted();
      if (!value_exports.Check(ZaiWebSchema, args))
      ...
      const budgetMs = (web.timeout_seconds ?? 90) * 1e3;
      const started = Date.now();
      const remaining = () => Math.max(0, budgetMs - (Date.now() - started));
      for (const query2 of web.search_query ?? []) {
        signal?.throwIfAborted();
        onUpdate?.({
          content: [{ type: "text", text: `Searching (zai): ${query2.q}\u2026` }],
        ...
        sections.push(`## Results for: ${query2.q}

${formatted.join("\n\n")}`);
      }
      for (const item of web.open ?? []) {
        signal?.throwIfAborted();
        const entry = /^https?:\/\//i.test(item.ref_id) ? { url: item.ref_id } : deps.refs.resolve(item.ref_id);
        if (!entry?.url)
          throw new Error(
        ...
        const response = await client.readUrl(
          ...
          { signal, timeoutMs: Math.min(9e4, remaining() || 9e4) }
        );
        requestIds.push(response.requestId ?? response.data.id ?? "");
...
`;
      ...
      const fullOutputPath = truncated.truncated ? await deps.artifacts.saveText(ctx.sessionId, full).catch(() => void 0) : void 0;
      return {
        content: [
          {
            type: "text",
...
// packages/capabilities/search_web/zai/src/index.ts
var index_default = {
  manifest,
  create: (services) => ({
    tool: zaiWebTool({
