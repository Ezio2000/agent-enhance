...
// packages/capabilities/search_web/openai/src/manifest.ts
var manifest = {
  apiVersion: 1,
...
function redact(text2, secrets = []) {
  ...
  for (const secret of expanded.filter(Boolean).sort((a, b) => b.length - a.length))
    text2 = text2.replaceAll(secret, "[REDACTED]");
  return text2.replace(/Bearer\s+[^\s"\\]+/gi, "Bearer [REDACTED]").replace(/eyJ[\w-]+\.[\w-]+\.[\w-]+/g, "[REDACTED JWT]").replace(/[\u0000-\u0008\u000b-\u001f\u007f]/g, "");
}
  ...
  async post(path, body, options) {
    ...
    try {
      ...
      if (!response.ok) {
        let payload;
        try {
          payload = await readJSON(response, 64 * 1024, signal);
        } catch {
          payload = {};
        }
        signal.throwIfAborted();
      ...
      if (options.signal?.aborted)
        throw new ProtocolError(
          "Operation cancelled. A remote image job may still finish; it was not retried."
        );
      if (signal.aborted)
        throw new ProtocolError(
          "Operation timed out. A remote image job may still finish; it was not retried."
        );
      if (error instanceof ProtocolError) throw error;
      throw new ProtocolError(
        `OpenAI request failed: ${redact(error instanceof Error ? error.message : String(error), credentialSecrets(auth)).slice(0, 1200)}`
      );
    } finally {
      clearTimeout(timer);
...
function integer(value, name, min, max) {
  if (value !== void 0 && (!Number.isInteger(value) || value < min || value > max))
    throw new Error(`${name} must be an integer between ${min} and ${max}.`);
}

// packages/capabilities/search_web/openai/src/validation.ts
...
function validateDomain(domain) {
  if (!/^(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$/i.test(domain) || domain.length > 253)
    throw new Error(
      `Invalid domain ${JSON.stringify(domain)}: use a bare hostname, without URL scheme, path or wildcard.`
    );
}
function validateSearchRequest(request) {
  ...
  if (!operations.some((x) => x?.length))
    throw new Error(
      "Provide at least one search_query, image_query, open, click, find, screenshot, finance, weather, sports or time operation."
    );
  for (const operation of operations)
    if (operation !== void 0)
      integer(
        ...
        "operation count",
        1,
        operation === c.search_query || operation === c.image_query ? 4 : 10
      );
  if ((c.search_query?.length ?? 0) > 4) throw new Error("search_query allows at most 4 queries per call.");
  if (c.search_query?.length === 4 && (c.response_length ?? "short") === "short")
    throw new Error("4 search queries require response_length: medium or long.");
  for (const q of [...c.search_query ?? [], ...c.image_query ?? []]) {
    requireText(q.q, "query", 4e3);
  ...
  for (const operation of [
    ...
    if (/^[a-z][a-z\d+.-]*:/i.test(operation.ref_id) && !/^https?:\/\//i.test(operation.ref_id))
      throw new Error("ref_id must be an HTTP(S) URL or a search reference ID.");
  }
  for (const o of c.open ?? []) integer(o.lineno, "lineno", 0, 1e7);
  for (const o of c.click ?? []) integer(o.id, "link id", 0, 1e7);
  for (const o of c.find ?? []) requireText(o.pattern, "pattern", 4e3);
  for (const o of c.screenshot ?? []) integer(o.pageno, "pageno", 0, 1e5);
  for (const o of c.finance ?? []) {
    requireText(o.ticker, "ticker", 100);
    if (!["equity", "fund", "crypto", "index"].includes(o.type))
      throw new Error("Invalid finance asset type.");
    if (o.market !== void 0 && !/^(?:[A-Z]{3})?$/.test(o.market))
      throw new Error("market must be ISO alpha-3, OTC, or empty for crypto.");
  }
  for (const o of c.weather ?? []) {
    requireText(o.location, "location", 300);
    validateDate(o.start, "start");
    integer(o.duration, "duration", 1, 365);
  }
  for (const o of c.sports ?? []) {
    if (o.tool !== void 0 && o.tool !== "sports") throw new Error("Invalid sports tool.");
    if (!["schedule", "standings"].includes(o.fn)) throw new Error("Invalid sports function.");
    if (!["nba", "wnba", "nfl", "nhl", "mlb", "epl", "ncaamb", "ncaawb", "ipl"].includes(o.league))
      throw new Error("Invalid sports league.");
    for (const key of ["team", "opponent", "locale"])
      if (o[key] !== void 0) requireText(o[key], key, 100);
    validateDate(o.date_from, "date_from");
    validateDate(o.date_to, "date_to");
  ...
  for (const o of c.time ?? [])
    if (!/^[+-](?:[01]\d|2[0-3]):[0-5]\d$/.test(o.utc_offset))
      throw new Error("utc_offset must be \xB1HH:MM (hours 00\u201323, minutes 00\u201359).");
  for (const domains of [
    request.settings?.filters?.allowed_domains,
    request.settings?.filters?.blocked_domains
  ]) {
    if (domains && domains.length > 100)
      throw new Error("Domain filters allow at most 100 domains per list.");
    for (const domain of domains ?? []) validateDomain(domain);
  }
  const location = request.settings?.user_location;
  if (location?.country && !/^[A-Z]{2}$/.test(location.country))
    throw new Error("country must be a two-letter uppercase ISO code.");
  ...
  integer(request.max_output_tokens, "max_output_tokens", 256, 2e4);
}

// packages/capabilities/search_web/openai/src/client.ts
var WebClient = class {
  http;
  constructor(resolveAuth, fetchImpl = fetch) {
  ...
  async search(request, options = {}) {
    ...
    return this.http.post("alpha/search", request, {
      signal: options.signal,
      timeoutMs: options.timeoutMs ?? 9e4,
      consume: async (response, signal, id, secrets) => {
        ...
        if (!isRecord(data) || typeof data.output !== "string")
          throw new ProtocolError("Search response is missing output.");
        if (data.results != null && !Array.isArray(data.results))
          throw new ProtocolError("Search results must be an array.");
        return data;
      }
    });
  }
...
// packages/capabilities/search_web/openai/src/output.ts
...
function truncateText(text2) {
  ...
  return { text: output, truncated: output.length < text2.length };
}

// packages/capabilities/search_web/openai/src/output.ts
var WebOutputStore = class extends ArtifactDirectories {
  async saveText(sessionId, text2) {
    const directory = await this.directory(sessionId);
    const path = join2(directory, "search.txt");
    const file = await open(path, "wx", 384);
    try {
      await file.writeFile(text2, "utf8");
    } finally {
      await file.close();
    }
    return path;
...
function Match2(schema, base, ref2) {
  if (IsId(schema)) {
    const result = MatchId(schema, base, ref2);
    if (!guard_exports.IsUndefined(result))
      return result;
  }
  if (IsAnchor(schema)) {
    const result = MatchAnchor(schema, base, ref2);
    if (!guard_exports.IsUndefined(result))
      return result;
  }
  if (IsDynamicAnchor(schema)) {
    const result = MatchDynamicAnchor(schema, base, ref2);
    if (!guard_exports.IsUndefined(result))
      return result;
  }
  return MatchHash(schema, base, ref2);
}
function FromArray2(schema, base, ref2) {
  return schema.reduce((result, item) => {
    const match = FromValue2(item, base, ref2);
    return !guard_exports.IsUndefined(match) ? match : result;
  }, void 0);
}
function FromObject2(schema, base, ref2) {
  return guard_exports.Keys(schema).reduce((result, key) => {
    const match = FromValue2(schema[key], base, ref2);
    return !guard_exports.IsUndefined(match) ? match : result;
  }, void 0);
}
function FromValue2(schema, base, ref2) {
  const nextBase = IsSchemaObject(schema) && IsId(schema) ? new URL(schema.$id, base.href) : base;
  if (IsSchemaObject(schema)) {
    const result = Match2(schema, nextBase, ref2);
    if (!guard_exports.IsUndefined(result))
      return result;
  }
  if (guard_exports.IsArray(schema))
    return FromArray2(schema, nextBase, ref2);
  if (guard_exports.IsObject(schema))
    return FromObject2(schema, nextBase, ref2);
  return void 0;
}
function Ref(schema, ref2) {
...
var Stack = class {
  ...
  Ref(ref2) {
    return __classPrivateFieldGet(this, _Stack_instances, "m", _Stack_FromContext).call(this, ref2) ?? __classPrivateFieldGet(this, _Stack_instances, "m", _Stack_FromRef).call(this, ref2);
  }
  // ----------------------------------------------------------------
  // RecursiveRef
  // ----------------------------------------------------------------
...
}, _Stack_FromContext = function _Stack_FromContext2(ref2) {
  return guard_exports.HasPropertyKey(this.context, ref2.$ref) ? this.context[ref2.$ref] : void 0;
}, _Stack_FromRef = function _Stack_FromRef2(ref2) {
  const root = this.schema;
  return !ref2.$ref.startsWith("#") ? resolve_exports.Ref(root, ref2.$ref) : resolve_exports.Ref(this.Base(), ref2.$ref);
};
...
function Ref2(ref2, options) {
  return memory_exports.Create({ ["~kind"]: "Ref" }, { $ref: ref2 }, options);
}
...
function IntrinsicOrCall(ref2, parameters) {
  return guard_exports.IsEqual(ref2, "Array") ? _Array_(parameters[0]) : guard_exports.IsEqual(ref2, "Capitalize") ? CapitalizeDeferred(parameters[0]) : guard_exports.IsEqual(ref2, "ConstructorParameters") ? ConstructorParametersDeferred(parameters[0]) : guard_exports.IsEqual(ref2, "Evaluate") ? EvaluateDeferred(parameters[0]) : guard_exports.IsEqual(ref2, "Exclude") ? ExcludeDeferred(parameters[0], parameters[1]) : guard_exports.IsEqual(ref2, "Extract") ? ExtractDeferred(parameters[0], parameters[1]) : guard_exports.IsEqual(ref2, "Index") ? IndexDeferred(parameters[0], parameters[1]) : guard_exports.IsEqual(ref2, "InstanceType") ? InstanceTypeDeferred(parameters[0]) : guard_exports.IsEqual(ref2, "Lowercase") ? LowercaseDeferred(parameters[0]) : guard_exports.IsEqual(ref2, "NonNullable") ? NonNullableDeferred(parameters[0]) : guard_exports.IsEqual(ref2, "Omit") ? OmitDeferred(parameters[0], parameters[1]) : guard_exports.IsEqual(ref2, "Parameters") ? ParametersDeferred(parameters[0]) : guard_exports.IsEqual(ref2, "Partial") ? PartialDeferred(parameters[0]) : guard_exports.IsEqual(ref2, "Pick") ? PickDeferred(parameters[0], parameters[1]) : guard_exports.IsEqual(ref2, "Readonly") ? ReadonlyObjectDeferred(parameters[0]) : guard_exports.IsEqual(ref2, "KeyOf") ? KeyOfDeferred(parameters[0]) : guard_exports.IsEqual(ref2, "Record") ? RecordDeferred(parameters[0], parameters[1]) : guard_exports.IsEqual(ref2, "Required") ? RequiredDeferred(parameters[0]) : guard_exports.IsEqual(ref2, "ReturnType") ? ReturnTypeDeferred(parameters[0]) : guard_exports.IsEqual(ref2, "Uncapitalize") ? UncapitalizeDeferred(parameters[0]) : guard_exports.IsEqual(ref2, "Uppercase") ? UppercaseDeferred(parameters[0]) : CallConstruct(Ref2(ref2), parameters);
}
...
function Resolve(defs, ref2) {
  return ref2 in defs ? IsRef2(defs[ref2]) ? Resolve(defs, defs[ref2].$ref) : defs[ref2] : Never();
}
...
function FromCyclic(defs, ref2) {
  const target = CyclicTarget(defs, ref2);
...
function FromCyclic3(defs, ref2) {
  ...
  const result = Cyclic(memory_exports.Assign(defs, { [ref2]: partial }), ref2);
  return result;
}
...
// packages/capabilities/search_web/openai/src/history.ts
function textContent(content) {
  if (typeof content === "string") return content;
  if (!Array.isArray(content)) return "";
  return content.filter(isRecord).filter((x) => x.type === "text" && typeof x.text === "string").map((x) => x.text).join("\n");
}
function recentSearchInput(messages) {
  const visible = messages.filter(
    (m) => (m.role === "user" || m.role === "assistant") && textContent(m.content)
  );
  ...
  let assistantBudget = 4e3;
  return visible.slice(start, end + 1).flatMap((m) => {
    let text2 = textContent(m.content);
    if (m.role === "assistant") {
      text2 = text2.slice(0, assistantBudget);
      assistantBudget -= text2.length;
    } else text2 = text2.slice(0, 16e3);
    if (!text2) return [];
    return [
      {
        type: "message",
        role: m.role,
        content: [{ type: m.role === "user" ? "input_text" : "output_text", text: text2 }]
      }
    ];
  });
}
...
// packages/transports/openai/src/schema.ts
...
// packages/capabilities/search_web/openai/src/schema.ts
var domainList = typebox_exports.Array(text("Bare domain, e.g. openai.com, without scheme or path", 253), {
  minItems: 1,
  maxItems: 100
});
var query = object({
  q: text("Search query"),
  recency: typebox_exports.Optional(
    typebox_exports.Integer({ minimum: 0, maximum: 36500, description: "Limit to the last N days" })
  ),
  domains: typebox_exports.Optional(domainList)
});
var ref = text("An HTTP(S) URL or an unchanged reference ID from a previous search_web result", 8192);
var WebSchema = object({
  search_query: typebox_exports.Optional(typebox_exports.Array(query, { minItems: 1, maxItems: 4 })),
  image_query: typebox_exports.Optional(typebox_exports.Array(query, { minItems: 1, maxItems: 4 })),
  open: typebox_exports.Optional(
    typebox_exports.Array(object({ ref_id: ref, lineno: typebox_exports.Optional(typebox_exports.Integer({ minimum: 0 })) }), {
      minItems: 1,
      maxItems: 10
    })
  ),
  click: typebox_exports.Optional(
    typebox_exports.Array(
      object({ ref_id: ref, id: typebox_exports.Integer({ minimum: 0, description: "Link number in an opened page" }) }),
      { minItems: 1, maxItems: 10 }
    )
  ),
  find: typebox_exports.Optional(
    typebox_exports.Array(object({ ref_id: ref, pattern: text("Text to find") }), { minItems: 1, maxItems: 10 })
  ),
  screenshot: typebox_exports.Optional(
    typebox_exports.Array(
      object({
        ref_id: ref,
        pageno: typebox_exports.Integer({ minimum: 0, description: "Zero-based PDF page number" })
      }),
      { minItems: 1, maxItems: 10 }
    )
  ),
  finance: typebox_exports.Optional(
    typebox_exports.Array(
      object({
        ticker: text("Ticker symbol", 100),
        type: choices(["equity", "fund", "crypto", "index"]),
        market: typebox_exports.Optional(
          typebox_exports.String({
            maxLength: 3,
            pattern: "^(?:[A-Z]{3})?$",
            description: "ISO alpha-3 country code, OTC, or empty string for crypto"
          })
        )
      }),
      { minItems: 1, maxItems: 10 }
  ...
  weather: typebox_exports.Optional(
    typebox_exports.Array(
      object({
        location: text("Country, Area, City", 300),
        start: typebox_exports.Optional(
          typebox_exports.String({ pattern: "^\\d{4}-\\d{2}-\\d{2}$", description: "YYYY-MM-DD; default today" })
        ),
        duration: typebox_exports.Optional(
          typebox_exports.Integer({
            minimum: 1,
            maximum: 365,
            description: "Number of days; default 7. Forecast coverage depends on backend."
          })
        )
      }),
      { minItems: 1, maxItems: 10 }
  ...
  sports: typebox_exports.Optional(
    typebox_exports.Array(
      object({
        fn: choices(["schedule", "standings"]),
        league: choices(["nba", "wnba", "nfl", "nhl", "mlb", "epl", "ncaamb", "ncaawb", "ipl"]),
        team: typebox_exports.Optional(text("Common team alias", 100)),
        opponent: typebox_exports.Optional(text("Opponent alias", 100)),
        date_from: typebox_exports.Optional(typebox_exports.String({ pattern: "^\\d{4}-\\d{2}-\\d{2}$" })),
        date_to: typebox_exports.Optional(typebox_exports.String({ pattern: "^\\d{4}-\\d{2}-\\d{2}$" })),
        num_games: typebox_exports.Optional(typebox_exports.Integer({ minimum: 1, maximum: 100 })),
        locale: typebox_exports.Optional(text("Locale, e.g. en-US", 100))
      }),
      { minItems: 1, maxItems: 10 }
    )
  ),
  time: typebox_exports.Optional(
    typebox_exports.Array(
      object({
        utc_offset: typebox_exports.String({
          pattern: "^[+-](?:[01]\\d|2[0-3]):[0-5]\\d$",
          description: "UTC offset, e.g. +08:00"
        })
      }),
      { minItems: 1, maxItems: 10 }
    )
  ),
  response_length: typebox_exports.Optional(
    choices(["short", "medium", "long"], "Default short; 4 search queries require medium or long")
  ),
  search_context_size: typebox_exports.Optional(
    choices(["low", "medium", "high"], "Amount of search context, not an exact token budget")
  ),
  external_web_access: typebox_exports.Optional(
    typebox_exports.Union([typebox_exports.Boolean(), choices(["cached", "indexed", "live"])], {
      description: "Default true (live); false for cache only; indexed restricts live fetches to indexed URLs"
    })
  ),
  filters: typebox_exports.Optional(
    object({ allowed_domains: typebox_exports.Optional(domainList), blocked_domains: typebox_exports.Optional(domainList) })
  ),
  user_location: typebox_exports.Optional(
    object({
      country: typebox_exports.Optional(
        typebox_exports.String({ pattern: "^[A-Z]{2}$", description: "Two-letter ISO country code" })
      ),
      city: typebox_exports.Optional(text("Approximate city", 200)),
      region: typebox_exports.Optional(text("Approximate region", 200)),
      timezone: typebox_exports.Optional(text("IANA timezone, e.g. Asia/Shanghai", 100))
    })
  ),
  image_settings: typebox_exports.Optional(
    object({
      max_results: typebox_exports.Optional(typebox_exports.Integer({ minimum: 1, maximum: 50 })),
      caption: typebox_exports.Optional(typebox_exports.Boolean())
    })
  ),
  max_output_tokens: typebox_exports.Optional(
    typebox_exports.Integer({ minimum: 256, maximum: 2e4, description: "Search service output budget; default 6000" })
  ),
  include_context: typebox_exports.Optional(
    typebox_exports.Boolean({
      description: "Default true: send only the last two user text messages and intervening assistant text; no files, tool results or reasoning"
    })
  ),
  timeout_seconds: typebox_exports.Optional(typebox_exports.Integer({ minimum: 5, maximum: 300, description: "Default 90" }))
});

// packages/capabilities/search_web/openai/src/tool.ts
function searchRequest(args, ctx) {
  const {
    search_query,
    image_query,
    open: open2,
  ...
  return {
    id: ctx.sessionId,
    model: "gpt-5.6-luna",
    input: args.include_context === false ? void 0 : recentSearchInput(ctx.history ?? []),
    commands: {
      search_query,
      image_query,
      open: open2,
    ...
    settings: {
      ...
      user_location: user_location ? { type: "approximate", ...user_location } : void 0,
      filters,
      image_settings,
      search_context_size
    },
    max_output_tokens: args.max_output_tokens ?? 6e3
  };
}
function webTool(deps) {
  return {
    name: "search_web",
    label: "OpenAI Web",
    ...
    async execute(_callId, args, signal, onUpdate, ctx) {
      signal?.throwIfAborted();
      if (!value_exports.Check(WebSchema, args))
      ...
      const result = await deps.client(ctx).search(request, { signal, timeoutMs: (args.timeout_seconds ?? 90) * 1e3 });
      signal?.throwIfAborted();
      const truncated = truncateText(result.data.output);
      const fullOutputPath = truncated.truncated ? await deps.artifacts.saveText(request.id, result.data.output) : void 0;
      return {
        content: [
          {
            type: "text",
            text: truncated.text + (fullOutputPath ? `
...
// packages/capabilities/search_web/openai/src/index.ts
var index_default = {
  manifest,
  create: (services) => ({
    tool: webTool({
